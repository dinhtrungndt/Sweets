...
const ChangeObjects = ({cancel, itemPosts, navigation}) => {
  ...
  const handleOptionSelect = option => {
    ...
    if (selectedObject) {
      const selectedId = selectedObject._id;
  ...
  // console.log('>>>.. objectsID Id:', objectsID);

  const handleChangeObjects = async () => {
    try {
  ...
  return (
    <View style={styles.T}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.text_editPosts}>Chỉnh sửa quyền riêng tư</Text>
        <TouchableOpacity onPress={handleChangeObjects}>
          <Text
            style={[
              styles.text_save,
              {color: objectsID ? '#22b6c0' : '#c4c4c4'},
            ]}>
            Xong
          </Text>
        </TouchableOpacity>
