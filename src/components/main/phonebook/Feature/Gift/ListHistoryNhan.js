...
const TabBar1 = ({ active, onPress }) => {
  return (
    <TouchableOpacity
      style={[styles.tabItem, active && styles.activeTabItem]}
      onPress={onPress}
    >
      <Text style={styles.tabText}>Thiệp sinh nhật</Text>
    </TouchableOpacity>
  );
}
...
export const Tab1Content = () => {
  ...
  const handleNavigate = (item) => {
    navigation.navigate('ReviewSeen', {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await AxiosInstance().get(`birthday/idReceiver/${userId}`);
      ...
      const senderDetailsPromises = invitations.map(async invitation => {
        const senderId = invitation.idSender; // Lấy idSender từ thông tin lời mời
